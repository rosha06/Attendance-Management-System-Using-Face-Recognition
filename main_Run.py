...
def manually_fill():
    ...
    def fill_attendance():
        ...
        try:
            ...
            messagebox.showerror("Database Error", f"Error creating table: {ex}")
        ...
        if subb == '':
            ...
            def attf():
                import subprocess
                attendance_path = os.path.join(os.getcwd(), "Attendance", "Manually Attendance")
                os.makedirs(attendance_path, exist_ok=True)
                subprocess.Popen(f'explorer "{attendance_path}"')
...
# for choose# for choose subject and fill attendance
def subjectchoose():
    def Fillattendances():
        sub = tx.get()
        ...
        if time.time() < future:
            if sub == '':
                ...
                try:
                    ...
                    messagebox.showerror("CV2 Error", f"Error loading face recognizer: {e}")
                    return
                except FileNotFoundError:
                    print("Model file not found. Please train the model first.")
                    messagebox.showerror("File Not Found", "Model file not found. Please train the model first.")
                    return
                ...
                timeStamp = datetime.datetime.fromtimestamp(
                    ts).strftime('%H:%M:%S')
                Hour, Minute, Second = timeStamp.split(":")
                fileName = os.path.join("Attendance", Subject + "_" + date + "_" + Hour + "-" + Minute + "-" + Second + ".csv")
                attendance = attendance.drop_duplicates(
                    ['Enrollment'], keep='first')
                print(attendance)
                try:
                    attendance.to_csv(fileName, index=False)
                    print(f"Attendance file created successfully at: {os.path.abspath(fileName)}")
                except Exception as e:
                ...
                try:
                    ...
                    print("Error: pymysql module not found. Please install it using 'pip install pymysql'")
                    # You might want to add a return statement here if you want to exit the function
                except pymysql.Error as e:
                    print(f"Error connecting to MySQL database: {e}")
                    # You might want to add a return statement here if you want to exit the function
...
def admin_panel():
    ...
    def log_in():
        ...
        stored_hash = b'$2b$12$9vXy1Qo5vZT8Ux8Uk8Aq1OyOX3zXy8Ux8Uk8Aq1OyOX3z'  # Example hash

        if username == 'admin' and check_password(password, stored_hash):
            win.destroy()
    ...
    Login = tk.Button(win, text="Log In", fg="black", bg="SkyBlue1", width=20,
                      height=2, activebackground="Red", command=log_in, 
                      font=('times', 15, 'bold'))
    ...
    with open(cs, newline="") as file:
        ...
        for r, row in enumerate(reader):
            for c, col in enumerate(row):
                label = tk.Label(root, width=10, height=1, fg="black", 
                                 font=('times', 15, 'bold'),
...
def trainimg():
    ...
    try:
        ...
        print(f"Error in getImagesAndLabels: {e}")
    ...
    try:
        ...
        Notification.configure(text="Error training the model", bg="Red", width=50, font=('times', 18, 'bold'))
        Notification.place(x=350, y=400)
        return
